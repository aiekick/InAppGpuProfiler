...
// https://www.shadertoy.com/view/3sf3WN
...
vec4 get(sampler2D sam, vec2 g, vec2 s, float x, float y, float z)
{
    vec2 v = (g + vec2(x,y)) * z;
    if (v.x < 0.) v.x = s.x;
    if (v.y < 0.) v.y = s.y;
...
{
    vec4 l 	= 	get(sam, g, s, -1. ,  0., 1.);
	vec4 lt = 	get(sam, g, s, -1. ,  1., 1.);
	vec4 t 	= 	get(sam, g, s,  0. ,  1., 1.);
	vec4 rt = 	get(sam, g, s,  1. ,  1., 1.);
	vec4 r 	= 	get(sam, g, s,  1. ,  0., 1.);
	vec4 rb = 	get(sam, g, s,  1. , -1., 1.);
	vec4 b 	= 	get(sam, g, s,  0. , -1., 1.);
	vec4 lb = 	get(sam, g, s, -1. , -1., 1.);
	vec4 c 	= 	get(sam, g, s,  0. ,  0., 1.);
...
void main() {
    ...
    vec2 uv = g/s * 0.5;
    float cc = get(iChannel0, g, s, 0., 0., 2.0).r;
    float cc2 = get(iChannel0, g, s, cos(a) * 0.8, sin(a) * 0.8, 2.0).r;
